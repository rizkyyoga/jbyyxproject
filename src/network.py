...
import socket, wx, thread, sys, struct
from packets import *
...
def RecvPacket(sock):
    ...
    size_data=sock_data='';recv_size=8
    id = -1
    while total_len<size:
        ...
        if id == -1:
            if len(sock_data)>=8:
                size_data+=sock_data
                id,size=struct.unpack('>ii', size_data[:8])
                recv_size=size
    ...
    return [id,size,''.join(total_data)]"""
...
class Network():
    ...
    def OnConnect(self):
        ...
        self.Write(ConnectPacket(self._nick, self._frame._engine.GetVersion()).Build())
